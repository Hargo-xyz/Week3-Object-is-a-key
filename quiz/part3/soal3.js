...
function countProfit(shoppers) {
    var listBarang = [
        ['Sepatu Stacattu'/*0*/, 1500000/*1*/, 10/*2*/],//0
        ['Baju Zoro', 500000, 2],//1
        ['Sweater Uniklooh', 175000, 1]//2
    ...
    let hasil = [
        { product: 'Sepatu Stacattu', shoppers: [], leftOver: 10, totalProfit: 0 },//0
        { product: 'Baju Zoro', shoppers: [], leftOver: 2, totalProfit: 0 },//1
        { product: 'Sweater Uniklooh', shoppers: [], leftOver: 1, totalProfit: 0 }//2
    ];
    for (let i = 0; i < listBarang.length; i++) {
        for (let j = 0; j < shoppers.length; j++) {
            if (shoppers[j].product === listBarang[i][0]) {
                if (hasil[i].leftOver - shoppers[j].amount >= 0){
                    hasil[i].shoppers.push(shoppers[j].name)    
                    hasil[i].leftOver = hasil[i].leftOver - shoppers[j].amount
                    hasil[i].totalProfit = (listBarang[i][2] - hasil[i].leftOver) * listBarang[i][1]
                }
            }
        }
    }
...
console.log(countProfit([
    ...
    { name: 'Vanessa', product: 'Sepatu Stacattu', amount: 10 }, { name: 'Rani', product: 'Sweater Uniklooh', amount: 1 },
